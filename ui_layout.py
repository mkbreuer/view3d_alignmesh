...
def draw_ui_panel_align_mesh(context, layout):
    ...
    if am_global.align_typ == 'align':                                                       
        col.operator("tpc_ot.mesh_align_help", text='', icon ='INFO').mode='align'
        col.label(text='Align: Direct')
    else:
        col.operator("tpc_ot.mesh_align_help", text='', icon ='INFO').mode='flatten'
    ...
    if am_global.align_typ == 'align':
        ...
        if am_global.use_axis_align == False:
        ...
        row.prop(am_global, 'use_axis_align' , text='', icon='AXIS_TOP')
        ...
        button_align_x = icons.get("icon_align_x") 
        props = row.operator("tpc_ot.snapflat_modal", text="X", icon_value=button_align_x.icon_id)
        props.mode='flatten_x'

        button_align_y = icons.get("icon_align_y")
        props = row.operator("tpc_ot.snapflat_modal",text="Y", icon_value=button_align_y.icon_id)
        props.mode='flatten_y'

        button_align_z = icons.get("icon_align_z")
        props = row.operator("tpc_ot.snapflat_modal",text="Z", icon_value=button_align_z.icon_id)
        ...
        if not looptools_state[0]:
            ...
            props = row.operator("tpc_ot.snapflat_modal", text="LPT Flatten*", icon='TRIA_DOWN_BAR')
            ...
            props = row.operator("tpc_ot.snapflat_modal", text="Region: Selection", icon='RESTRICT_SELECT_OFF')
            props.mode='snap_for_select'

            props = row.operator("tpc_ot.snapflat_modal", text="Region: UV Seam", icon='UV_FACESEL')
            props.mode='snap_for_uvs'

            props = row.operator("tpc_ot.snapflat_modal", text="Region: Mark Sharp", icon='SHADING_BBOX')
